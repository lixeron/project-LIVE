...
interface HardwarePreloaderProps {
  progress: number; // Linked directly to THREE.DefaultLoadingManager
  hardwareWarmed: boolean; // True when WebGL compiles the scenes
...
export const HardwarePreloader: React.FC<HardwarePreloaderProps> = ({
  ...
  const isSystemReady = progress >= 100 && hardwareWarmed;
  ...
  // WebGL & Fluid Simulation Lifecycle
  useEffect(() => {
    ...
    // 1. CPU Fluid Simulation Grid (64x64 resolution for extreme performance)
    const GRID_W = 64;
    const GRID_H = 64;
    ...
    const tick = () => {
      ...
      // (This is a safety guarantee so no noise/bleed calculation pushes edge cells above 0)
      for (let cx = 0; cx < GRID_W; cx++) {
  ...
  // Compute scroll-linked layout fades & blur-dissolves (0.0 to 0.1)
  const fadeFactor = experienceLaunched ? Math.min(1, scrollProgress / 0.1) : 0;
  const preloaderOpacity = 1 - fadeFactor;
  const preloaderBlur = fadeFactor * 24;

  const subtitleOpacity = experienceLaunched ? Math.max(0, 1 - scrollProgress / 0.05) : 0;
  const devIdOpacity = experienceLaunched ? Math.max(0, 0.45 - scrollProgress / 0.1) : 0.45;
  ...
  return (
    <div
      style={{
        ...
        filter: `blur(${preloaderBlur}px)`,
        WebkitFilter: `blur(${preloaderBlur}px)`,
        zIndex: scrollProgress >= 0.1 ? -1 : 9999,
    ...
    >
      {/* A. WebGL Watercolor fluid dynamics simulation canvas */}
      <canvas
        ref={canvasRef}
        style={{
          position: 'absolute',
      ...
      >
        <AnimatePresence>
          {!experienceLaunched && (
            ...
            >
              Preparing Next Phase Implementation
            </div>
          )}
        </AnimatePresence>
      </div>

      {/* C. Centered Layout Acronym & Poet definitions track Grid */}
      <div
        style={{
          display: 'flex',
          flexDirection: 'column',
      ...
      >
        {/* Acrostic Column Layout (Aligned Letters and Definitions Vertically) */}
        <div
          style={{
            display: 'grid',
        ...
        {/* Scroll-Linked Subtitle Indicator */}
        <div
          style={{
            opacity: mounted ? 1 : 0,
            transition: 'opacity 1.2s cubic-bezier(0.16, 1, 0.3, 1)',
      ...
      >
        ...
        >
          {/* Subtle developer tag locked middle bottom */}
          <div
            style={{
              fontFamily: "'Outfit', sans-serif",
              fontSize: '0.62rem',
